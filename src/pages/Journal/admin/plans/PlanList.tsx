...
export default function HomePage() {
  ...
  const roleData = useApi<I_ROLE_TYPE_LIST>({
    api: `${loanApi.getAllPlan}?page=${page}&limit=${perPage}&q=${search}`,
    key: 'getAllPlan',
    value: [page, perPage],
    options: {
      enabled: true,
  ...
  const handelStatusUpdate = async (roleId: string) => {
    try {
      const result = await mutate.mutateAsync({
        api: `${loanApi?.updatePlanStatus}/${roleId}`,
        data: {
          id: id
        }
      });
                  ...
                  <TableBody>
                    {roleData?.data?.data?.docs?.map((role, index) => (
                      <TableRow key={role?._id}>
                        ...
                        <TableCell>{role?.planName}</TableCell>
